...
public static void main(String[] args)
{
    ...
    public void start()
    {
        ...
        placeTwo();
        do{
            drawBoard(); //method fills the canvas with light gray, draws the black lines between the squares, and draws all the pieces that have been placed on the board.
            resolve();
            placeTwo();
        }while((x=notDone())>0);
        drawBoard();
    ...
    int notDone(){
        ...
        for(moves=y=0;y<4;y++){
            for(x=0;x<4;x++){
                if(x<3&&board[y][x]==board[y][x+1]||y<3&&board[y][x]==board[y+1][x])
                {
                moves++;
                }
                else if(board[y][x]>2047)
                return -1;
            }
        }
